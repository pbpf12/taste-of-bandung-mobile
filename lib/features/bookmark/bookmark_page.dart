...
class _BookmarkPageState extends State<BookmarkPage> {
  Future<BookmarkEntry> fetchBookmarks(CookieRequest request) async {
    final response = await request.get('http://${EndPoints().myBaseUrl}/last_activities/');

    // Check for authentication errors
    if (response.containsKey('error') && response['error'] == 'User not authenticated') {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<BookmarkEntry>(
